...
int main(int agrc, char* argv[]) {
    ...
    double *a, *b, *res_sum;   // Исходные и результирующая матрицы
    ...
    if (proc_rank == 0) {
        ...
        res_sum = (double *) malloc(m * n * sizeof(double));
    ...
    // Вывод результатов
    if (proc_rank == 0) {
        printf("Общее время сложения параллельно: %5.5f s\n%d-process.\n", MPI_Wtime()-start_time, proc_size);
    }
