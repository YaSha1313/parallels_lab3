...
void MatrixSum_parall(int m, int n, float **a, float **b, float **res);

void MatrixSub_parall(int m, int n, float **a, float **b, float **res);
...
int main() {
    ...
    for (int i = 0; i < num; i++) {
        ...
        start = omp_get_wtime();
        MatrixSum_parall(m, n, a, b, res_sum);
    ...
    for (int i = 0; i < num; i++) {
        ...
        start = omp_get_wtime();
        MatrixSub_parall(m, n, a, b, res_sub);
...
void MatrixSum_parall(int m, int n, float **a, float **b, float **res) {
    ...
    #pragma omp parallel private (m, n, a, b, res) num_threads(4)
    for (int i = 0; i < m; ++i) {
        for (int j = 0; j < n; ++j) {
            // Вводим элемент с индексами i, j
            res[i][j] = a[i][j] + b[i][j];
